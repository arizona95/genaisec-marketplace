...
검증자를 추가할 때 이 파일은 안 고친다 — action/validators.json 에 항목만 넣으면 된다.
"""
...
def changed_assets(base_ref: str) -> list[str] | None:
    ...
    빨개지는 게 더 나쁘다. 다만 검증자 자체(action/)가 바뀌면 판정 기준이 바뀐 것이므로
    ...
    if any(f.startswith(("action/", ".github/")) for f in files):
        print("검증 기준(action/ · .github/)이 바뀌었습니다 — 전수로 돌립니다.")
        return None
